...
export async function processWhatsAppWebhook(body) {
  try {
    ...
    // Deduplication check using in-memory Set
    ...
    if (busError || !businesses || businesses.length === 0) {
      console.error('❌ Failed to fetch business for webhook:', busError?.message);
      return;
    }
    ...
    // State: BOOKED
    if (state === 'BOOKED') {
      console.log(`📊 State is BOOKED. Sending confirmation reminder.`);
      await sendWhatsAppMessage(
        fromPhone,
    ...
    // State: AWAITING_SLOT (slot choice)
    ...
    // States: NEW, QUALIFYING, READY_FOR_BOOKING
    console.log('🧠 Sending message to OpenAI for extraction...');
    const leadInfo = await extractLeadInfo(messageBody, state);
    console.log('🤖 AI Lead Extraction output:', JSON.stringify(leadInfo, null, 2));

    const updateData = {};
    if (leadInfo.property_type && leadInfo.property_type !== lead.property_type) {
      updateData.property_type = leadInfo.property_type;
    }
    if (leadInfo.budget && String(leadInfo.budget) !== lead.budget) {
      updateData.budget = String(leadInfo.budget);
    }
    ...
    if (Object.keys(updateData).length > 0) {
      ...
      const { data: updatedLead, error: upError } = await supabase
        ...
        .select()
        .single();
      if (!upError && updatedLead) {
        lead = updatedLead;
      }
    }

    // Check date selection from input (if READY_FOR_BOOKING state)
    let chosenDate = null;
    if (state === 'READY_FOR_BOOKING') {
      chosenDate = parseDateChoice(messageBody);
    }

    // Explicit date requested in current message
    if (!chosenDate && leadInfo.intent === 'BOOK_TOUR' && leadInfo.requested_tour_iso) {
      try {
        ...
        console.error('❌ Error parsing requested_tour_iso:', e);
      }
    }
    ...
    // Fully qualified lead -> request site visit
    if (lead.property_type && lead.budget) {
      console.log(`📊 Lead is fully qualified (Property: ${lead.property_type}, Budget: ${lead.budget}). Asking for site visit...`);
      await sendWhatsAppMessage(
        fromPhone,
    ...
    // Qualification clarification questions
    console.log(`📊 Lead is not fully qualified. Asking clarification questions.`);
    ...
    if (!lead.property_type && !lead.budget) {
      ...
      clarificationMsg = `Thanks! I've noted your budget of ${formatBudget(lead.budget)}. Could you tell me what type of property you're looking for (e.g. 2BHK, 3BHK, Villa)?`;
    } else {
      clarificationMsg = `Thanks! I've noted your preference for a ${lead.property_type}. Could you tell me your approximate budget?`;
    }
